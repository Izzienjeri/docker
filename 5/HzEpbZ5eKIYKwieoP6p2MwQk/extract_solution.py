...
def extract_solution(llm_response: str) -> list[tuple[str, str]]:
  ...
  if 'export default function PublicWrapper' not in code_solution and 'export default const PublicWrapper' not in code_solution:
        code_solution += '\n\nmodule.exports = {PublicWrapper};'

  return [('src/solution.tsx', code_solution)]